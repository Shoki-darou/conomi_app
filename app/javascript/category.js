$(function(){
  ...
  function categoryOption(category){
    var optionHtml = `<option value="${category.id}">${category.category_name}</option>`;
    return optionHtml;
  }
  // 親カテゴリー選択後のイベント
  $('#category-select-parent').on('change', function(){
    ...
    if (parentCategoryId == ''){
      ...
      $.ajax({
        url: '/questions/category_children',
        type: 'GET',
      ...
      .done(function(category_children){
        ...
        $('#error-category').before(`<div class="sell-collection_select " id="select-children-box">
                                        <label class="sell-collection_select__label" for="question_category_id">
                                          <select class="sell-collection_select__input" id="category-select-children" required="required" name="question[category_id]">
  ...
  $('.sell-container__content__details').on('change', '#category-select-children', function(){
    ...
    if (childrenCategoryId == ''){
      ...
      $.ajax({
        url: '/questions/category_grandchildren',
        type: 'GET',
      ...
      .done(function(category_grandchildren){
        ...
        $('#error-category').before(`<div class="sell-collection_select " id="select-grandchildren-box">
                                        <label class="sell-collection_select__label" for="question_category_id">
                                          <select class="sell-collection_select__input" id="category-select-grandchildren" required="required" name="question[category_id]">
                                            <option value="">選択して下さい</option>
                                            ${optionHtml}
                                          </select>
                                          <i class="fas fa-chevron-down"></i>
